...
<?php
	...
	if($cnt > 0) {
		...
		foreach ( $chk_sql as $row ) {
			$any_tel = preg_replace( "/[^0-9]/", "", $row['cs_tel'] );
			if( $any_tel == $new_tel ) {
				$chk_num++;
			}
		};
	
		$overlap_yn = ($chk_num == 0) ? 'N' : 'Y';
	...
	$value = array(':cs_name'=>$cs_name, ':cs_tel'=>$cs_tel, ':proc_id'=>$proc_id, ':proc_ip'=>$proc_ip, ':idx'=>$idx);
	$query = "
		UPDATE mt_db SET
			  cs_name = :cs_name
			, cs_tel = :cs_tel
