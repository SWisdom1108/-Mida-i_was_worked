...
<?php
	...
	foreach ( $sheetData as $row ){
		...
		$tmCode = trim($row[etExcelColumnString($lastColumns + 3)]);
		$fcCode = trim($row[etExcelColumnString($lastColumns + 4)]);
		...
		# 팀(부서)
		$tmCode = str_replace("TM", "", $tmCode);
		$value = array(':idx'=>$tmCode);
		$query = "SELECT idx FROM mt_member_team WHERE use_yn = 'Y' AND idx = :idx";
		$tmCode = view_pdo($query, $value)['idx'];
		if(!$tmCode){
			$failed['reason'] = "팀없음";
