function byteConvert(bytes) {
	...
	let size = bytes / Math.pow(1024, Math.floor(idx));
	size = Math.round(size * 10) / 10;

	return size + names[idx];
}
$(function(){
	...
	$("#bbsFile").change(function(){
		...
		$(item).find("div:nth-of-type(1)").find("input").attr("name", "files"+$('.bbsFileUploadListWrap .item').length);
	});
