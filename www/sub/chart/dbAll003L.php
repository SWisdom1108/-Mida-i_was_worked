<?php
	...
	# 콘텐츠설정
	$contentsTitle = "생산업체 DB통합통계";
	$contentsInfo = "생산업체별 업로드된 DB통계를 확인하실 수 있습니다.";

	# 콘텐츠 경로설정
	$contentsRoots = array();
	array_push($contentsRoots, "DB통합통계");
	array_push($contentsRoots, "생산업체");
	...
	while(true){
		...
		foreach($pmList as $pmCode){
			...
			$query = "
				SELECT
					  ( SELECT COUNT(*) FROM mt_db WHERE pm_code = '{$pmCode}' AND reg_date LIKE '{$newDate}%' ) AS uploadCnt
					, ( SELECT COUNT(*) FROM mt_db WHERE pm_code = '{$pmCode}' AND use_yn = 'Y' AND dist_code = 002 AND dist_date LIKE '{$newDate}%' ) AS distCnt
					, ( SELECT COUNT(*) FROM mt_db WHERE pm_code = '{$pmCode}' AND use_yn = 'N' AND edit_date LIKE '{$newDate}%' ) AS deleteCnt
					, ( SELECT COUNT(*) FROM mt_db WHERE pm_code = '{$pmCode}' AND use_yn = 'Y' AND dist_code = 001 AND reg_date LIKE '{$newDate}%' ) AS stockCnt
				FROM dual
			";
			...
			// $thisPmData = view_sql("
			// 	SELECT
			// 		  ( SELECT COUNT(*) FROM mt_db_chart_log WHERE type_name = 'upload' AND code_type = 'pm' AND code_value = '{$pmCode}' AND reg_date LIKE '{$newDate}%' ) AS uploadCnt
			// 		, ( SELECT COUNT(*) FROM mt_db_chart_log WHERE type_name = 'dist' AND code_type = 'pm' AND code_value = '{$pmCode}' AND reg_date LIKE '{$newDate}%' ) AS distCnt
			// 		, ( SELECT COUNT(*) FROM mt_db_chart_log WHERE type_name = 'delete' AND code_type = 'pm' AND code_value = '{$pmCode}' AND reg_date LIKE '{$newDate}%' ) AS deleteCnt
			// 		, ( SELECT db_cnt FROM mt_db_chart_log WHERE type_name = 'stock' AND code_type = 'pm' AND code_value = '{$pmCode}' AND reg_date LIKE '{$newDate}%' ) AS stockCnt
			// 	FROM dual
			// ");
			...
			$chartData[$pmCode]["uploadCnt"] += $thisPmData["uploadCnt"];
			$chartData[$pmCode]["distCnt"] += $thisPmData["distCnt"];
			$chartData[$pmCode]["deleteCnt"] += $thisPmData["deleteCnt"];
			$chartData[$pmCode]["stockCnt"] += $thisPmData["stockCnt"];
		}
	...
	$andQuery = " WHERE 1=1";
	$andQuery .= " AND pm_code IN ( {$pmList} )";
...
?>
	...
	<div class="listWrap">
		<table>
			...
			<tbody>
				<?php foreach($chartData as $pmCode => $data){ ?>
					<tr>
						<td class="lp05"><?=$totalCnt?></td>
						<td class="lp05">PM<?=$pmCode?><br><?=dhtml($pmNameList[$pmCode])?></td>
						...
						<td class="stopProgram">
							<button type="button" class="typeBtn mini btnGray02" onclick='popupControl("open", "dayChart", "/sub/chart/dbAll003DayL?s_date=<?=$startDate?>&e_date=<?=$endDate?>&code=<?=$pmCode?>", "PM<?=$pmCode?> DB통합 일별통계");' style="max-width: 100%; float: none;">
								<i class="fas fa-chart-pie"></i>일별통계보기
							</button>
						</td>
						<td class="stopProgram">
							<button type="button" class="typeBtn mini btnGray02" onclick='popupControl("open", "dbList", "/sub/chart/dbAll003DBL?s_date=<?=$startDate?>&e_date=<?=$endDate?>&code=<?=$pmCode?>", "PM<?=$pmCode?> DB통합 상세목록");' style="max-width: 100%; float: none;">
