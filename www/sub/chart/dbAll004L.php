<?php
	...
	# 콘텐츠설정
	$contentsTitle = "{$customLabel["tm"]} DB통합통계";
	$contentsInfo = "{$customLabel["tm"]}별 업로드된 DB통계를 확인하실 수 있습니다.";

	# 콘텐츠 경로설정
	$contentsRoots = array();
	array_push($contentsRoots, "DB통합통계");
	array_push($contentsRoots, $customLabel["tm"]);
	...
	$pmList = [];
	$pmNameList = [];
	$value = array(':use_yn'=>'Y',':auth_code'=>'004');
	$query = "
		SELECT MT.*
	...
	while($row = $sql->fetch(PDO::FETCH_ASSOC)){
		...
		$pmNameList[$row['idx']] = $row["team_name"];
		$totalCnt++;
	}
	...
	while(true){
		...
		foreach($pmList as $pmCode){
			$query = "
				SELECT
					  ( SELECT COUNT(*) FROM mt_db WHERE tm_code = '{$pmCode}' AND use_yn = 'Y' AND dist_code = '002' AND dist_date LIKE '{$newDate}%' ) AS distCnt
					, ( SELECT COUNT(*) FROM mt_db WHERE tm_code = '{$pmCode}' AND use_yn = 'N' AND edit_date LIKE '{$newDate}%' ) AS deleteCnt
				FROM dual
			";
	...
	$andQuery = " WHERE 1=1";
	$andQuery .= " AND tm_code IN ( {$pmList} )";
			...
			<tbody>
				<?php foreach($chartData as $pmCode => $data){ ?>
					<tr>
						...
						<td class="lp05">TM<?=$pmCode?><br><?=dhtml($pmNameList[$pmCode])?></td>
						<td class="lp05"><?=number_format($data["distCnt"])?></td>
						<td class="lp05"><?=number_format($data["deleteCnt"])?></td>
						<td class="stopProgram">
							<button type="button" class="typeBtn mini btnGray02" onclick='popupControl("open", "dayChart", "/sub/chart/dbAll004DayL?s_date=<?=$startDate?>&e_date=<?=$endDate?>&code=<?=$pmCode?>", "TM<?=$pmCode?> DB통합 일별통계");' style="max-width: 100%; float: none;">
								<i class="fas fa-chart-pie"></i>일별통계보기
							</button>
						</td>
						<td class="stopProgram">
							<button type="button" class="typeBtn mini btnGray02" onclick='popupControl("open", "dbList", "/sub/chart/dbAll004DBL?s_date=<?=$startDate?>&e_date=<?=$endDate?>&code=<?=$pmCode?>", "TM<?=$pmCode?> DB통합 상세목록");' style="max-width: 100%; float: none;">
