<?php
	...
	if($_GET['code']){
		$code = $_GET['code'];
		$value = array(':idx'=>$_GET['code'], ':tm_code'=>$user['tm_code']);
		$query = "SELECT * FROM mt_member WHERE use_yn = 'Y' AND idx = :idx AND tm_code = :tm_code";
	...
	$codeInfo['m_name'] = dhtml($codeInfo['m_name']);

	# 메뉴설정
	$secMenu = "dbMyTeam";
	$trdMenu = ($code) ? "fc{$code}" : "all";
	$value = array(':idx'=>$user['tm_code']);
	$query = "SELECT team_name FROM mt_member_team WHERE idx = :idx";
	$user["tm_name"] = view_pdo($query, $value)["team_name"];
	
	# 콘텐츠설정
	$contentsTitle = ($code) ? "{$codeInfo['m_name']} DB분배통계" : "전체 DB분배통계";
	$contentsInfo = "현재 소속되어 있는 {$customLabel["tm"]}의 분배된 DB를 확인하실 수 있습니다.";
	...
	if($codeInfo){
		$andQuery .= " AND m_idx = '{$codeInfo["idx"]}'";
	}
		...
		<table>
			...
			<thead>
				<tr>
					<th rowspan="2">NO</th>
					<th rowspan="2">년월</th>
					<th rowspan="2"><?=dhtml($customLabel["tm"])?>정보</th>
			...
			<?php } else {?>
				...
				<?php foreach($chartDatas as $index => $val){ ?>
					...
					<tr>
						...
						<td class="lp05">TM<?=$user["tm_code"]?><br><?=dhtml($user['tm_name'])?></td>
						<td class="lp05">FC<?=$data[0]?><br><?=dhtml($data[1])?></td>
