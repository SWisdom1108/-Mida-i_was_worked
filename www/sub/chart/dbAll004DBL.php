<?php
	...
	$andQuery = " WHERE tm_code = '{$_GET["code"]}' AND date_format(dist_date, '%Y-%m-%d') >= date_format('{$startDate}', '%Y-%m-%d') AND date_format(dist_date, '%Y-%m-%d') <= date_format('{$endDate}', '%Y-%m-%d')";
	// $andQuery = " AND code_type = 'tm' AND code_value = '{$_GET["code"]}' AND date_format(reg_date, '%Y-%m-%d') >= date_format('{$startDate}', '%Y-%m-%d') AND date_format(reg_date, '%Y-%m-%d') <= date_format('{$endDate}', '%Y-%m-%d')";
...
?>
	...
	<div class="listEtcWrap">
		...
		<div class="right">
			<a href="/excel/chart/dbAll004DBL?s_date=<?=$startDate?>&e_date=<?=$endDate?>&code=<?=$_GET["code"]?>" class="typeBtn btnGreen02" title="엑셀다운로드"><i class="fas fa-file-excel"></i>엑셀다운로드</a> 
		</div>
	</div>
