<?php
	...
	# 콘텐츠설정
	$contentsTitle = ($code) ? "{$codeInfo['team_name']} 덴트웹DB관리" : "전체 덴트웹DB관리";
	$contentsInfo = "{$customLabel["tm"]}의 덴트웹 DB를 관리하실 수 있습니다.";

	# 콘텐츠 경로설정
	$contentsRoots = array();
	array_push($contentsRoots, "덴트웹DB관리");
	...
	if($_GET["chart_num"]){
		$andQuery .= " AND MT.chart_num = '{$_GET["chart_num"]}'";
	}
	...
	# 검색값 정리
	$_SEARCH["md_name"] = " AND MT.md_idx IN ( SELECT idx FROM mt_member WHERE m_name LIKE '%{$_GET["value"]}%' )";
	$_SEARCH["dr_name"] = " AND MT.dr_idx IN ( SELECT idx FROM mt_member WHERE m_name LIKE '%{$_GET["value"]}%' )";
	search();
    $tmp = $andQuery;
    $join = "
	...
	<div class="listEtcWrap">
		...
		<div class="right">
			<select class="listSet" id="orderBy">
				...
				<option value="rcpt_date ASC">최근진료일 오름차순</option>
				<option value="rcpt_date DESC">최근진료일 내림차순</option>
			</select>
		...
		<table>
			...
			<?php
				...
				while($row = $sql->fetch(PDO::FETCH_ASSOC)){
					$value = array(':chart_num'=> $row['chart_num']);
					$query = "SELECT COUNT(*) AS cnt FROM mt_pay_log WHERE use_yn = 'Y' AND chart_num = :chart_num";
			...
			?>
				<tr class="" data-type="open" data-target="mod" data-url="/sub/db_dent/dbDentU?idx=<?=$row['idx']?>" data-name="DB정보" >
					<td>
