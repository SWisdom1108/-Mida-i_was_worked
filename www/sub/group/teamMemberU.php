...
?>
	...
	<div class="writeWrap">
		<form enctype="multipart/form-data" id="modFrm" data-ajax="/ajax/group/teamMemberUP" data-callback="/sub/group/teamMemberV?idx=<?=$view['idx']?>" data-type="수정">
			...
			<table>
				...
				<tbody>
					<tr>
						...
						<td>
							<select class="txtBox" name="tm_code">
								...
								<?php
									...
									$query = "
										...
										ORDER BY idx DESC
									";
									$team = list_pdo($query, $value);
									while($row = $team->fetch(PDO::FETCH_ASSOC)){
								?>
									<option value="<?=$row['idx']?>" <?=$row['idx'] == $view['tm_code'] ? "selected" : ""?>><?=dhtml($row['team_name'])?></option>
								<?php } ?>
							</select>
						</td>
						<th>담당관리자 여부<div class="miniGuideWrap" data-class="groupTeamMG"></div></th>
						<td>
							<input type="checkbox" class="toggle" name="mg_yn" id="mg_yn" <?=($view['mg_idx'] == $view['idx']) ? "checked" : ""?>>
	...
	<div class="dataBtnWrap">
		<div class="left">
			<a href="/sub/group/teamMemberV?idx=<?=$view['idx']?>" class="typeBtn btnGray02" title="취소"><i class="fas fa-arrow-left"></i>취소</a> 
		</div>
		<div class="right">
