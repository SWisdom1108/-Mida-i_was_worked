<?php
	...
	# 콘텐츠설정
	$contentsTitle = "나의 DB관리";
	$contentsInfo = "나에게 분배된 DB를 관리하실 수 있습니다.";
	...
	# 추가 쿼리문
	$andQuery .= " AND use_yn = 'Y' AND dist_code = '002' AND tm_code = '{$user['tm_code']}' AND m_idx = '{$user['idx']}'";
  ...
  if($_GET["chart_num"]){
	...
	if($_GET["grade_code"]){
		$andQuery .= " AND grade_code = '{$_GET["grade_code"]}'";
	}
...
<!--			<button type="button" class="typeBtn btnOrange big dbCsAllBtn"><i class="fas fa-edit"></i>상담일괄등록</button>-->
			<!-- <button type="button" class="typeBtn btnGray01 big popupBtn" data-type="open" data-target="write" data-url="/sub/db/dbMyW" data-name="DB 추가하기"><i class="fas fa-plus-circle"></i>DB추가</button> -->
		...
		<div class="right">
			...
			<?php if($user['excel_yn'] =="Y"){ ?>
				<button type="button" class="typeBtn btnGreen01" style="background-color: #fff; color: #b3b3b3; border-color: #b3b3b3;" onclick="popupControl('open', 'excel', '/sub/db/excel/dbSelectMy', '엑셀항목설정<span style=\'color: #999999; margin-left: 20px;font-size: 15px; font-weight: 100;\'> 원하시는 항목을 선택하여 엑셀을 다운로드 받으실 수 있습니다.</span>');"><i class="fas fa-file-excel"></i>엑셀항목설정</button> 
				<div class="line" style="width: 1px; height: 35px; float: left; background-color: #CCC; margin: 0 15px;"></div>
			<?php } ?>
<?php if($user['excel_yn'] =="Y"){ ?>
			<a href="/excel/db/dbMyL" class="typeBtn btnGreen02" title="엑셀다운로드"><i class="fas fa-file-excel"></i>엑셀다운로드</a> 
			<?php } ?>
		</div>
	</div>
		...
		<table>
			...
			<?php
				...
				while($row = $sql->fetch(PDO::FETCH_ASSOC)){
					...
					# 200624 최종상담값 가져오기
					$value = array(':use_yn'=>'Y',':db_idx'=>"'{$row['idx']}'");
					$query = "SELECT COUNT(*) AS cnt FROM mt_db_cs_log WHERE use_yn = :use_yn AND db_idx = :db_idx";
					$lastCsCnt = view_pdo($query, $value)['cnt'];

					$value = array(':use_yn'=>'Y',':db_idx'=>"'{$row['idx']}'");
					$query = "SELECT status_code FROM mt_db_cs_log WHERE use_yn = :use_yn AND db_idx = :db_idx ORDER BY idx DESC";
					...
					# 201118 오늘자 상담횟수
					$value = array(':use_yn'=>'Y',':db_idx'=>"{$row["idx"]}",':reg_date'=>date("Y-m-d").'%');
					$query = "SELECT COUNT(*) AS cnt FROM mt_db_cs_log WHERE use_yn = :use_yn AND db_idx = :db_idx AND reg_date LIKE :reg_date";
			...
			?>
				<tr class="rowMove popupBtn" data-type="open" data-target="mod" data-url="/sub/db/dbMyTeamU?idx=<?=$row['idx']?>" data-name="DB정보">
					...
					<td><?=($row['grade_name']) ? $row['grade_name'] : "-"?></td>
	...
	<form method="get">
		<div class="popupDetail">
            ...
            <div class="detailSearchElement">
						...
						<div class="detailSearchElement">
							<p class="detailElementTit">DB유입경로</p>
						...
						<div class="detailSearchElement">
							...
							<?php foreach($gradeArr as $val){ ?>
								<option value="<?=$val['grade_code']?>" <?=($_GET['grade_code'] == $val['grade_code']) ? "selected" : ""?>><?=$val['grade_name']?></option>
							<?php } ?>
							</select>
						</div>
