...
?>
	...
	<div class="dataBtnWrap">
		...
		<?php if($codeInfo['use_yn']=="Y" || !$code){ ?>
			<?php if($user['excel_yn'] =="Y"){ ?>
				<button type="button" class="typeBtn btnGreen01" style="background-color: #fff; color: #b3b3b3; border-color: #b3b3b3;" onclick="popupControl('open', 'excel', '/sub/db/excel/dbSelectAll', '엑셀항목설정<span style=\'color: #999999; margin-left: 20px;font-size: 15px; font-weight: 100;\'> 원하시는 항목을 선택하여 엑셀을 다운로드 받으실 수 있습니다.</span>');"><i class="fas fa-file-excel"></i>엑셀항목설정</button> 
		...
		<?php if($codeInfo['use_yn']=="Y" || !$code){ ?>
			<?php if($user['excel_yn'] =="Y"){ ?>
				<a href="/excel/db/dbAllL?code=<?=$code?>" class="typeBtn btnGreen02" title="엑셀다운로드"><i class="fas fa-file-excel"></i>엑셀다운로드</a> 
			<?php } ?>
		<?php } ?>
		</div>
	</div>
	...
	<style>
		...
		.popupDetailBody { overflow-y: auto; padding: 10px; max-height: calc(80vh - 80px); height: 430px; }
	</style>
