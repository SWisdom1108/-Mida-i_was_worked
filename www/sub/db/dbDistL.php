<?php
	...
	# 콘텐츠설정
	$contentsTitle = "DB통합관리";
	$contentsInfo = "생산된 DB를 업로드하여 전체 관리가 가능합니다.";
	...
	# 추가 쿼리문
	$andQuery .= " AND use_yn = 'Y' AND pm_code = '{$user['pm_code']}'";

	# 초기 정렬
	$_GET['orderBy'] = ($_GET['orderBy']) ? $_GET['orderBy'] : "made_date DESC";

	# 데이터 간단정리표
	$value = array(':pm_code'=>"{$user['pm_code']}",':pm_code2'=>"{$user['pm_code']}",':pm_code3'=>"{$user['pm_code']}");
	$query = "
		SELECT
			( SELECT COUNT(*) FROM mt_db WHERE use_yn = 'Y' AND pm_code = :pm_code ) AS totalCnt
			, ( SELECT COUNT(*) FROM mt_db WHERE use_yn = 'Y' AND pm_code = :pm_code2 AND reg_date LIKE '".date("Y-m-d")."%' ) AS todayCnt
			, ( SELECT COUNT(*) FROM mt_db WHERE use_yn = 'Y' AND pm_code = :pm_code3 AND dist_code = '002' ) AS distCnt
		FROM dual
	";
	...
	if($_GET["cs_status_date"]){
		$cs_status_date = $_GET["cs_status_date"];
		$andQuery .= " AND cs_status_date LIKE '%{$cs_status_date}%'";
	}

	if($_GET["dist_code"]){
		$dist_code = $_GET["dist_code"];
		$andQuery .= " AND dist_code = '{$dist_code}'";
	}

	if($_GET["overlap_yn"]){
		$overlap_yn = $_GET["overlap_yn"];
...
?>
	...
	<div class="dataInfoSimpleWrap">
		<div>
			...
			<div class="btnWrap">
				<button type="button" class="typeBtn btnRed" onclick="popupControl('open', 'excel', '/sub/db/excel/dbDist', 'DB 대량업로드');"><i class="fas fa-plus-circle"></i>DB업로드</button>
				<button type="button" class="typeBtn btnGray01 popupBtn" data-type="open" data-target="write" data-url="/sub/db/dbDistW" data-name="DB 추가하기" style="height: 35px; line-height: 35px; font-size: 15px; margin-top: 5px;"><i class="fas fa-plus-circle"></i>DB추가</button>
			</div>
		</div>
	</div>
		...
		<form method="get">
			<ul class="formWrap">
				...
				<li class="drag">
					...
					<input type="hidden" name="setDate" value="made">
	...
	<div class="dataBtnWrap">
		<div class="left">
			<button type="button" class="typeBtn btnMain big popupBtn" data-type="open" data-target="write" data-url="/sub/db/dbDistW" data-name="DB 추가하기"><i class="fas fa-plus-circle"></i>DB추가</button>
		...
		<div class="right">
			<?php if($user['excel_yn'] =="Y"){ ?>
				<button type="button" class="typeBtn btnGreen01" style="background-color: #fff; color: #b3b3b3; border-color: #b3b3b3;" onclick="popupControl('open', 'excel', '/sub/db/excel/dbSelectDist', '엑셀항목설정<span style=\'color: #999999; margin-left: 20px;font-size: 15px; font-weight: 100;\'> 원하시는 항목을 선택하여 엑셀을 다운로드 받으실 수 있습니다.</span>');"><i class="fas fa-file-excel"></i>엑셀항목설정</button> 
				<div class="line" style="width: 1px; height: 35px; float: left; background-color: #CCC; margin: 0 15px;"></div>
			<?php } ?>
			<button type="button" class="typeBtn btnGreen01" onclick="popupControl('open', 'excel', '/sub/db/excel/dbDist', 'DB 대량업로드');"><i class="fas fa-file-excel"></i>엑셀업로드</button> 
<?php if($user['excel_yn'] =="Y"){ ?>
			<a href="/excel/db/dbDistL" class="typeBtn btnGreen02" title="엑셀다운로드"><i class="fas fa-file-excel"></i>엑셀다운로드</a> 
			<?php } ?>
		</div>
	</div>
		...
		<table>
			...
			<?php
				...
				while($row = $sql->fetch(PDO::FETCH_ASSOC)){
					$value = array(':db_idx'=>$row['idx']);
					$query = "SELECT status_code FROM mt_db_cs_log WHERE use_yn = 'Y' AND db_idx = :db_idx ORDER BY idx DESC";
					$lastCsStatus = view_pdo($query, $value)['status_code'];
					if($lastCsStatus){
						$value = array(':status_code'=>$lastCsStatus);
						$query = "SELECT finish_yn FROM mc_db_cs_status WHERE status_code = :status_code";
						$lastCsStatus = view_pdo($query, $value)['finish_yn'];
					} else {
						$lastCsStatus = "N";
					}
			?>
				<?php if($row['dist_code'] == "001"){ ?>
					<tr class="rowMove popupBtn" data-type="open" data-target="mod" data-url="/sub/db/dbDistU?idx=<?=$row['idx']?>" data-name="DB정보">
				<?php }else{ ?>
					<tr class="rowMove popupBtn" data-type="open" data-target="mod" data-url="/sub/db/dbDistV?idx=<?=$row['idx']?>" data-name="DB정보">
				<?php }?>
					<td>
						<?php if($row['dist_code'] == "001"){ ?>
					...
					<td class="lp05"><?=($row['dist_code'] =="002") ? "<span style='color:#006cfc; font-weight: 500;'>분배완료</span>" : "미분배"?></td>
					<td class="lp05" style="line-height: 15px;">
						<?php $row['cs_status_name'] = dhtml($row['cs_status_name']) ?>
						<?=($row['cs_status_name']) ? "<b style='color: {$row['color']};'>{$row["cs_status_name"]}</b>" : "-"?>
						<br><span style="font-size: 12px; color: #AAA;"><?=($row['cs_status_date']) ? date("Y-m-d", strtotime($row['cs_status_date'])) : "-"?></span>
					</td>
					<td>
						<?php if($cmpyView == "Y" && $row['dist_code'] == "002"){ ?>
							<?=(mb_strlen($row['cs_name']) > 1) ? mb_substr($row['cs_name'],0,1,'utf-8')."**" : dhtml($row['cs_name'])?>
						<?php }else{ ?>
							<?=($row['cs_name']) ? dhtml($row['cs_name']) : "-"?>
						<?php } ?>	
					</td>
					<td class="lp05">						
						<?php if($cmpyView == "Y" && $row['dist_code'] == "002"){ ?>
							<?=(mb_strlen($row['cs_tel']) > 1) ? mb_substr($row['cs_tel'],0,1,'utf-8')."*******" : $row['cs_tel']?>
						<?php }else{ ?>
...
<?php
  ...
  $csStatusArr = [];
  $value = array('' => '');
  $query = "SELECT status_code, status_name FROM mc_db_cs_status WHERE use_yn = 'Y'";
  $cs_status = list_pdo($query, $value);

  while($row = $cs_status->fetch(PDO::FETCH_ASSOC)){
    $thisdatas = [];
    $thisdatas['status_name'] = $row['status_name'];
    $thisdatas['status_code'] = $row['status_code'];
    
    array_push($csStatusArr, $thisdatas);
  }
			...
			<div class="popupDetail2">
				...
				<div class="popupDetailBody" style="height: 430px;">
					<div style="width: 100%; float:left; padding:10px; margin: 0 auto;">
						...
						<div class="detailSearchElement">
							<p class="detailElementTit">중복여부</p>
							<select class="detailSearchSelect" name="overlap_yn">
								<option value="">선택</option>
                <option value="Y" <?=($_GET['overlap_yn'] == "Y") ? "selected" : ""?>>중복</option>
                <option value="N" <?=($_GET['overlap_yn'] == "N") ? "selected" : ""?>>미중복</option>
							</select>
						</div>
              ...
              <div class="dateInputWrap">
                <div class="sDate detailSearchInput">
                  ...
                  <i class="fas fa-calendar-alt"></i>
						...
						<div class="detailSearchElement">
							<p class="detailElementTit">분배여부</p>
							<select class="detailSearchSelect" name="dist_code">
								<option value="">선택</option>
                <option value="002" <?=($_GET['dist_code'] == "002") ? "selected" : ""?>>분배완료</option>
                <option value="001" <?=($_GET['dist_code'] == "001") ? "selected" : ""?>>미분배</option>
							</select>
						</div>

						<div class="detailSearchElement">
							<p class="detailElementTit">상담상태</p>
							<select class="detailSearchSelect" name="cs_status_code">
								<option value="">선택</option>
                <?php foreach($csStatusArr as $val){ ?>
                  <option value="<?=$val['status_code']?>" <?=($_GET['cs_status_code'] == $val['status_code']) ? "selected" : ""?>><?=$val['status_name']?></option>
                <?php } ?>
							</select>
						</div>

            <div class="detailSearchElement">
              <p class="detailElementTit">상담일</p>
              <div class="dateInputWrap">
                <div class="sDate detailSearchInput">
                  <input type="text" class="txtBox" name="cs_status_date" id="cs_status_date" dateonly placeholder="상담일" style="border-radius: 5px;" value="<?=$_GET['cs_status_date']?>" autocomplete="off">
                  <i class="fas fa-calendar-alt"></i>
                </div>
              </div>
            </div>
