		...
		<form enctype="multipart/form-data">
					...
					<tr>
						...
						<td>
							...
							<?php
								...
								$query = "SELECT * FROM mt_member_team WHERE use_yn = :use_yn";
								$sql = list_pdo($query, $value);
								while($row = $sql->fetch(PDO::FETCH_ASSOC)){
									$value = array(':tm_code'=>$row['idx']);
									$query = "SELECT GROUP_CONCAT(idx) AS idxs, GROUP_CONCAT(m_name) AS names FROM mt_member WHERE use_yn = 'Y' AND tm_code = :tm_code";
									$thisFCList = view_pdo($query, $value);
			...
			<div class="listWrap">
				...
				<div class="info" style="color: #333;">
					<b>총 <span id="dbTotalCnt" style="color: #DC3333"><?=number_format(count($data))?></span>개가 분배 대기중</b>에 있습니다.
				</div>
				<table>
					<colgroup>
						<col width="4%">
					...
					<thead>
						<tr>
							...
							<th rowspan="2" style="border-left: 1px solid #FFF;">삭제</th>
						</tr>
						<tr>
					...
					<tbody>
					<?php
						$value = array(':use_yn'=>'Y',':dist_code'=>'001');
						$query = "
							SELECT MT.*
						...
						while($row = $sql->fetch(PDO::FETCH_ASSOC)){
							$i++;
					?>
						<tr <?=($row['overlap_yn'] == "Y") ? "class='overClass'" : ""?>>
							<td class="lp05"><?=$i?></td>
							<td class="stopProgram"><?=($row['overlap_yn']=="Y") ? "<span style='color:#CD3333; font-weight: 500;'>중복</span>" : "미중복"?><?php if($row['overlap_yn']=="Y") { ?><?php } ?></td>
						...
						<?php foreach($columnArr as $val){ ?>
							<td class="<?=($val['type'] == "file")? "stopProgram" : ""?>"><?php
								if($val['type'] == "file"){
									if($row["{$val['code']}"]){
	...
	<script type="text/javascript">
		...
		$(function(){
			...
			$("#submitBtn").click(function(){
				...
				var overCnt = $(".listWrap tbody > .overClass").length;
				...
				if(overCnt<1){
					if (confirm("선택한 DB를 모두 분배하시겠습니까?") == true){
				...
				else if (confirm("중복DB가 "+overCnt+"개 있습니다.\n선택한 DB를 모두 분배하시겠습니까?") == true){
					$("#loadingWrap").fadeIn(350, function(){
						$.ajax({
							url : "/ajax/db/dbDistTM",
							type : "POST",
