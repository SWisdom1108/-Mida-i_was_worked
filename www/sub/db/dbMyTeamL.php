<?php
	...
	if($_GET['code']){
		$code = $_GET['code'];
		$value = array(':use_yn'=>'Y',':idx'=>"{$_GET['code']}",':tm_code'=>"{$user['tm_code']}");
		$query = "SELECT * FROM mt_member WHERE use_yn = :use_yn AND idx = :idx AND tm_code = :tm_code";
		$codeInfo = view_pdo($query, $value);
		if(!$codeInfo){
			www("/sub/db/dbMyTeamL");
			return false;
		}
	}
	...
	# 메뉴설정
	$secMenu = "dbMyTeam";
	$trdMenu = ($code) ? "fc{$code}" : "all";
	
	# 콘텐츠설정
	$contentsTitle = ($code) ? "{$codeInfo['m_name']} DB분배관리" : "전체 DB분배관리";
	$contentsInfo = "현재 소속되어 있는 {$customLabel["tm"]}의 분배된 DB를 관리하실 수 있습니다.";

	# 콘텐츠 경로설정
	$contentsRoots = array();
	array_push($contentsRoots, "DB분배관리");
	array_push($contentsRoots, ($code) ? $codeInfo['m_name'] : "전체보기");
	...
	$query = "
		SELECT
			...
			, ( SELECT COUNT(*) FROM mt_db {$andQuery} AND order_by_date LIKE '".date("Y-m-d")."%' ) AS todayCnt
		FROM dual
	";
	...
	$query = "SELECT * FROM mc_db_cs_status WHERE use_yn = :use_yn AND sms_yn = :sms_yn ORDER BY sort ASC";
...
?>
		...
		<div>
			...
			<div class="conWrap">
				<ul class="dataCntList">
					...
					<li>
						<span class="label">오늘의 분배DB</span>
						<span class="value"><?=number_format($dashboard['todayCnt'])?></span>
					</li>
				</ul>
			</div>
			<div class="btnWrap">
				<button type="button" class="typeBtn btnGray01 popupBtn" data-type="open" data-target="write" data-url="/sub/db/dbTeamW?code=<?=$tmCode?>" data-name="DB 추가하기" style="height: 35px; line-height: 35px; font-size: 15px; margin-top: 5px;"><i class="fas fa-plus-circle"></i>DB추가</button>
			</div>
		</div>
	</div>
	...
	<div class="dataBtnWrap">
		<div class="left">
			<button type="button" class="typeBtn btnMain big dbDistBtn"><i class="fas fa-exchange-alt"></i>분배하기</button>
			<div class="line"></div>
<!--			<button type="button" class="typeBtn btnOrange big dbCsAllBtn"><i class="fas fa-edit"></i>상담일괄등록</button>-->
		...
		<div class="right">
			...
			<?php if($user['excel_yn'] =="Y"){ ?>
				<button type="button" class="typeBtn btnGreen01" style="background-color: #fff; color: #b3b3b3; border-color: #b3b3b3;" onclick="popupControl('open', 'excel', '/sub/db/excel/dbSelectMyTeam', '엑셀항목설정<span style=\'color: #999999; margin-left: 20px;font-size: 15px; font-weight: 100;\'> 원하시는 항목을 선택하여 엑셀을 다운로드 받으실 수 있습니다.</span>');"><i class="fas fa-file-excel"></i>엑셀항목설정</button> 
				<div class="line" style="width: 1px; height: 35px; float: left; background-color: #CCC; margin: 0 15px;"></div>
			<?php } ?>
<?php if($user['excel_yn'] =="Y"){ ?>
			<a href="/excel/db/dbMyTeamL?code=<?=$code?>" class="typeBtn btnGreen02" title="엑셀다운로드"><i class="fas fa-file-excel"></i>엑셀다운로드</a> 
			<?php } ?>
		</div>
	</div>
		...
		<table>
			...
			<?php
				...
				while($row = $sql->fetch(PDO::FETCH_ASSOC)){
					...
					$query = "SELECT COUNT(*) AS cnt FROM mt_db_cs_log WHERE use_yn = 'Y' AND db_idx = :db_idx";
					...
					# 201118 오늘자 상담횟수
					$value = array(':db_idx'=>$row['idx']);
					$query = "SELECT COUNT(*) AS cnt FROM mt_db_cs_log WHERE use_yn = 'Y' AND db_idx = :db_idx AND reg_date LIKE '".date("Y-m-d")."%'";
			...
			?>
				<tr class="rowMove popupBtn" data-type="open" data-target="mod" data-url="/sub/db/dbMyTeamU?idx=<?=$row['idx']?>" data-name="DB정보">
					<td>
					...
					<td class="lp05 phoneQrBtn" data-tel="<?=$row['cs_tel']?>"><?=($row['cs_tel']) ? $row['cs_tel'] : "-"?></td>
				<?php foreach($columnArr as $val){ ?>
	...
	<script type="text/javascript">
		$(function(){
			...
			$(".dbDistBtn").click(function(){
				...
				if(!item.length){
					alert("분할 DB를 선택해주시길 바랍니다.");
					return false;
				}

				popupControl("open", "dist", "/sub/db/dbMyTeamChange", "DB분배하기", false, "선택한 DB를 담당자에게 분배하실 수 있습니다.");
			});
