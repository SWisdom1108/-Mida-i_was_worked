...
?>
	...
	<div class="writeWrap">
		<form enctype="multipart/form-data">
			...
			<table>
				...
				<tbody>
					<tr>
						<th class="important">상담상태 선택</th>
						<td>
							<select class="txtBox" name="status_code">
							<?php
							...
							?>
								<option value="<?=$row['status_code']?>"><?=dhtml($row['status_name'])?></option>
							<?php } ?>
							</select>
						</td>
					</tr>
					<tr>
						<th class="important">상담내용</th>
						<td>
							<input type="text" class="txtBox" name="memo">
						</td>
					</tr>
				</tbody>
			</table>
			...
			<div class="listWrap">
				<div></div>
				<div class="info">
					총 <span id="dbTotalCnt"><?=number_format(count($data))?>개</span>의 DB가 상담일괄등록 대기중에 있습니다.
				</div>
				<table>
					<colgroup>
						<col width="4%">
					...
					?>
						...
						<?php foreach($columnArr as $val){ ?>
							<td style="<?=($val['type'] == "file")? "stopProgram" : ""?>"><?php
								if($val['type'] == "file"){
									if($row["{$val['code']}"]){
	...
	<div id="popupBtnWrap">
		...
		<button type="button" class="typeBtn btnGray02 popupCloseBtn" data-target="csAll">취소</button>
	</div>
	...
	<script type="text/javascript">
		$(function(){
			...
			$("#submitBtn").click(function(){
				...
				if(!$("select[name='status_code']").val()){
					alert("상담상태를 선택해주시길 바랍니다.");
					return false;
				}
				
				$("#loadingWrap").fadeIn(350, function(){
					$.ajax({
						url : "/ajax/db/csLogAllWP",
						type : "POST",
						data : datas,
						success : function(result){
							switch(result){
								case "success" :
									...
									break;
								case "fail" :
									alert("알 수 없는 이유로 일괄등록을 실패하였습니다.");
