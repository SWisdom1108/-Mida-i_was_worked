...
?>
	...
	<div class="dataBtnWrap">
		...
		<div class="right">
			<!-- <a href="/excel/db/dbDentL?code=<?=$code?>" class="typeBtn btnGreen02" title="엑셀다운로드"><i class="fas fa-file-excel"></i>엑셀다운로드</a>  -->
		</div>
	</div>
		...
		<table>
			...
			<?php
				...
				while($row = $sql->fetch(PDO::FETCH_ASSOC)){
					...
					# 200624 최종상담값 가져오기
					$value = array(':chart_num'=> $row['chart_num'], ':dr_idx'=>$user['idx']);
					$query = "SELECT COUNT(*) AS cnt FROM mt_pay_log WHERE use_yn = 'Y' AND chart_num = :chart_num AND dr_idx = :dr_idx";
			...
			?>
				<tr class="" data-type="open" data-target="mod" data-url="/sub/db/dbDentU?idx=<?=$row['idx']?>" data-name="DB정보" >
					...
					<td class="stopProgram">
                        <i class="fas fa-coins csBtn click dbPayBtn" style="font-size: 16px; color: <?=$site['main_color']?>;" onclick='popupControl("open", "payLogL", "/sub/db/payLogL?chart_num=<?=$row['chart_num']?>&dr_idx=<?=$row['dr_idx']?>&dr_idx=<?=$row['dr_idx']?>", "결제기록");'></i><br>
