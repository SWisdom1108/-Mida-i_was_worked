<?php
	...
	# 추가 쿼리문
	$andQuery .= " AND use_yn = 'N' AND dist_code = '002' AND tm_code = '{$user['tm_code']}'";
...
?>
	...
	<div class="dataBtnWrap">
		...
		<div class="right">
			<?php if($user['excel_yn'] =="Y"){ ?>
				<button type="button" class="typeBtn btnGreen01" style="background-color: #fff; color: #b3b3b3; border-color: #b3b3b3;" onclick="popupControl('open', 'excel', '/sub/db/excel/trashSelecteMy', '엑셀항목설정<span style=\'color: #999999; margin-left: 20px;font-size: 15px; font-weight: 100;\'> 원하시는 항목을 선택하여 엑셀을 다운로드 받으실 수 있습니다.</span>');"><i class="fas fa-file-excel"></i>엑셀항목설정</button> 
				<div class="line" style="width: 1px; height: 35px; float: left; background-color: #CCC; margin: 0 15px;"></div>
			<?php } ?>
			<?php if($user['excel_yn'] =="Y"){ ?>
			<a href="/excel/db/trashMyL" class="typeBtn btnGreen02" title="엑셀다운로드"><i class="fas fa-file-excel"></i>엑셀다운로드</a> 
			<?php }?>
		</div>
	</div>
	...
	<div class="listWrap">
		<table>
			...
			<?php if(!$totalCnt){ ?>
				<tr>
					<td colspan="<?=($columnCnt + 5)?>">조회된 데이터가 존재하지 않습니다.</td>
				</tr>
			<?php } ?>
			</tbody>
		</table>
