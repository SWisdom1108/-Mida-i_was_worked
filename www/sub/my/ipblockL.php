...
?>
	<div class="ipblock">
		<ul>
			<a href = "/sub/my/ipblockL"><li class="active">등록 IP 차단</li></a>
			<div style="position : absolute;left : 0;top: 1px; border: 1px solid #000;width: 180px;height: 40.5px;z-index: 2; border-bottom: 0;opacity: 0.2;pointer-events: none; top:0px"></div>
			<a href = "/sub/my/ipblockL2"><li style="min-width:180px; float:left; border-top: 1px solid #ccc; border-right: 1px solid #ccc;text-align: center; padding: 10px 0px; color:#999; font-size:14px;">로그인 실패 IP 차단</li></a>
		</ul>
	</div>
<div class="ipbox">
	<div class="writeWrap" style="margin-bottom: 50px;">
		<form action="/ajax/my/permitWP.php" id="permitIpFrm" method="post">
			<table style="width: 90%;">
				...
				<tbody>
					<tr>
						<th class="label">차단 IP</th>
						<td class="value">
							<input class="txtBox" type="text" id="blockTel" name="blockTel" placeholder="접근을 차단할 IP 입력 ex)110.11.11.111" maxlength="45">
					...
					<tr>
						...
						<td class="value">
							<input class="txtBox" type="text" id="ipName" name="ipName" placeholder="접근을 차단할 IP 이름입력 ex)홍길동" maxlength="20">
						</td>
					</tr>
				</tbody>
			</table>
		...
		<table>
			...
			?>
				<tr class="rowMove" data-type="open" data-target="mod"  data-name="접근 차단 IP정보">
					...
					<td class="lp05"><?=dhtml($row['block_ip'])?></td>
				...
				if(confirm("해당 IP를 추가하시겠습니까?")){
					$("#loadingWrap").fadeIn(350, function(){
                        ...
                        success : function(result){
	                        	switch(result){
	                        		case "success" :
										alert("차단 IP를 추가하였습니다.");
										loadingClose();
										window.location.reload();
										break;
									case "fail" :
										alert("알수없는 오류로 차단 IP 추가를 실패하였습니다.");
										loadingClose();
										break;
									default :
										alert(result);
