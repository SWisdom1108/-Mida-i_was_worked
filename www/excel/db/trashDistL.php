<?php
	...
	$objPHPExcel->setActiveSheetIndex(0)
		...
		->setCellValue(etExcelColumnString(2)."1", "생산일자")
		->setCellValue(etExcelColumnString(3)."1", "{$customLabel["cs_name"]}")
		->setCellValue(etExcelColumnString(4)."1", "{$customLabel["cs_tel"]}");
	...
	foreach($columnArr as $index => $val){
		$objPHPExcel->setActiveSheetIndex(0)
			->setCellValue(etExcelColumnString(5 + $index)."1", $val['name']);
	}
	...
	$value = array(':use_yn'=>'N',':dist_code'=>'001',':pm_code'=>$user['pm_code']);
	$query = "
			SELECT COUNT(*) as cnt
			FROM mt_db MT
			WHERE use_yn = :use_yn
	...
	while($row = $result->fetch(PDO::FETCH_ASSOC)){
		$objPHPExcel->setActiveSheetIndex(0)
			...
			->setCellValue(etExcelColumnString(2).$rows, date("Y-m-d", strtotime($row['made_date'])))
			->setCellValue(etExcelColumnString(3).$rows, $row['cs_name'])
			->setCellValue(etExcelColumnString(4).$rows, $row['cs_tel']);
			...
			foreach($columnArr as $index => $val){
				...
				$objPHPExcel->setActiveSheetIndex(0)
					->setCellValue(etExcelColumnString(5 + $index).$rows, $cs);
			}
