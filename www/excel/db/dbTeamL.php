<?php
	...
	$objPHPExcel->setActiveSheetIndex(0)
		...
		->setCellValue(etExcelColumnString(3)."1", "등록일시")
		->setCellValue(etExcelColumnString(4)."1", "고객등급")
		->setCellValue(etExcelColumnString(5)."1", "차트번호")
	...
	$lastColum++;
	$objPHPExcel->setActiveSheetIndex(0)
		->setCellValue(etExcelColumnString($lastColum)."1", "담당{$customLabel["tm"]}(TM)");
	...
	$andQuery .= ($_GET['code']) ? " AND tm_code = '{$_GET['code']}'" : "";
	...
	while($row = $result->fetch(PDO::FETCH_ASSOC)){
		$objPHPExcel->setActiveSheetIndex(0)
			...
			->setCellValue(etExcelColumnString(3).$rows, $row["reg_date"])
			->setCellValue(etExcelColumnString(4).$rows, ($row['grade_code'] == "000")? "없음": $row['grade_name'])
			->setCellValue(etExcelColumnString(5).$rows, ($row['chart_num']? $row['chart_num'] : "-"))
			->setCellValue(etExcelColumnString(6).$rows, $row['cs_name'])
			->setCellValue(etExcelColumnString(7).$rows, $row['cs_tel']);
		...
		$thisColumnRow++;
		$objPHPExcel->setActiveSheetIndex(0)
			->setCellValue(etExcelColumnString($thisColumnRow).$rows, ($row['tm_name']) ? "{$row['tm_name']}(TM{$row['tm_code']})" : "-");
