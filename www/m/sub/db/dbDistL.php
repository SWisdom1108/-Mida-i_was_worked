<?php
	...
	switch($user['auth_code']){
		case "004" :
			$code = ($_GET['code']) ? $_GET['code'] : view_sql("SELECT idx FROM mt_member WHERE use_yn = 'Y' AND tm_code = '{$user['tm_code']}' AND auth_code = '005' ORDER BY idx DESC LIMIT 0, 1")['idx'];
			$codeInfo = view_sql("SELECT * FROM mt_member WHERE use_yn = 'Y' AND idx = '{$code}' AND auth_code = '005'");
			$codeInfo['name'] = $codeInfo['company_name'];
			$codeSql = list_sql("SELECT * FROM mt_member WHERE use_yn = 'Y' AND tm_code = '{$user['tm_code']}' AND auth_code = '005' ORDER BY idx DESC");
			$codeNameType = 'm_name';
			break;
	}
	...
	# 추가 쿼리문
	$andQuery .= " AND use_yn = 'Y' AND dist_code = '002' AND tm_code = '{$user['tm_code']}' AND m_idx = '{$code}'";
...
?>
	...
	<div class="distCodeListWrap">
		...
		<?php foreach ( $codeSql as $row ){ ?>
			...
			<li class="<?=$class?>"><a href="/m/sub/db/dbDistL?code=<?=$row['idx']?>"><?=$row[$codeNameType]?></a></li>
		<?php } ?>
		</ul>
	</div>
	...
	<div class="dataSectionWrap">
		...
		<div class="dataListWrap">
			...
			?>
				<ul onclick="www('/m/sub/db/dbDistV?idx=<?=$row['idx']?>');">
